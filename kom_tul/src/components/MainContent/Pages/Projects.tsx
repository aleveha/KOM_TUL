            ...
            {props.projectGroup.map(project => {
                return (
                    <div className="projectsBlock border" key={project.name}>
                        {Object.entries(project).map((elem) => ProjectBlock({
                            info: elem[1],
                            key: elem[0]
                        }))}
                    </div>
...
const Projects = () => {
    const {t} = useTranslation();
    const solvedProjects: Array<IProject> = t('main.projects.solvedProjects', {returnObjects: true});
    const archiveProjects: Array<IProject> = t('main.projects.projectsArchive', {returnObjects: true});

    return (
        <div className="projects">
            <div className="projectsInfo">
                <ProjectsGroup projectGroup={solvedProjects} title={t('main.projects.labels.labels.0')} />
                <ProjectsGroup projectGroup={archiveProjects} title={t('main.projects.labels.labels.1')} />
            </div>
        </div>
    );
};
